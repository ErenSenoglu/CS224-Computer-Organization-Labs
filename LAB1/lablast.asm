...
interval:
	...
	li $v0,5
	syscall
	move $s3,$v0  		#input of larger value
	li $v0,4
	la $a0,ask_ints
	syscall
	li $v0,5
	syscall
	move $s4,$v0 		#input of smaller value
	jal ifunction
	move $a0,$v0
	li $v0,1
	syscall
...
loop2:	beq $t4,$t5,done2	#Loop condition
	...
	slt $t6,$s2,$v0		#Checking if value is lt input
	beq  $t6,$0,iterate
...
loop3:	beq $t4,$t5,done3	#Loop condition
	...
	slt $t6,$s2,$s3		#Checking if value is lt input
	
	slt $t7,$s4,$s2		#Checking if value is gt input 2
...
loop4:	beq $t4,$t5,done2	#Loop condition
	...
	rem $t6,$s2,$s5		#Checking if value is lt input
	bne $t6,$0,iterate4
